...
type OrderItemPack struct {
	...
	Order_item []models.OrderItem `json:"order_item" bson:"order_item"`
}
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Error while fetching order items",
			})
			return
		}
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Error while fetching order item",
			})
			return
		}
...
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var validationErr error = validate.Struct(OrderItemPack)
		...
		orderItemsToBeInserted := make([]interface{}, 0)
		order.Table_id = OrderItemPack.Table_id
		order_id := OrderItemOrderCreator(order)

		for _, orderItem := range OrderItemPack.Order_item {
			orderItem.Order_id = order_id
			orderItem.Order_item_id = primitive.NewObjectID()
			orderItemID, _ := primitive.ObjectIDFromHex(orderItem.Order_item_id)
			orderItem.Order_item_id = orderItemID.Hex()
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Error while inserting order items",
			})
			return
		}
...
func UpdateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.BindJSON(&orderItem); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"error": "Error while binding order item",
			})
			return
		}
		...
		if validateErr != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"error": "Error while validating order item",
			})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Error while updating order item",
			})
			return
		}
